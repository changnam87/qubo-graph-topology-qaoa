"""
RQ3 sub-QUBO solution-quality preservation pilot.
...
Purpose:
    ...
    solution quality after lifting them back to the original variable space.

Important:
    Classical solvers are used only as reference tools.
    This does not claim classical or quantum superiority.
"""
...
def main() -> None:
    ...
    for idx, row in extraction_df.iterrows():
        ...
        # Exact solve sub-QUBO.
        sub_exact = brute_force_solve_qubo(
            subqubo,
            max_variables=20,
        )

        # Lift sub-QUBO solution into original variable space.
        ...
        # Normalize by reference magnitude to avoid scale-only interpretation.
        ...
        if (idx + 1) % 12 == 0:
            print(f"completed {idx + 1}/{len(extraction_df)}")

    out = pd.DataFrame(rows)
    out.to_csv(OUTPUT_PATH, index=False)

    print("=" * 80)
    print("RQ3 sub-QUBO solution-quality metrics saved")
    print("=" * 80)
