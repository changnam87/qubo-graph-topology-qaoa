"""
RQ2 single-instance ordering test with fixed initial layout.

This script compares variable ordering strategies on one QUBO instance
and one topology while imposing a fixed initial layout.

This is important because, without a fixed initial layout, Qiskit may choose
its own layout and absorb the ordering effect.

Input:
    results/rq1_extended/graph_metrics.csv

Output:
    results/rq2_pilot/single_instance_ordering_comparison_fixed_layout.csv
"""
...
OUTPUT_PATH = RESULTS_DIR / "single_instance_ordering_comparison_fixed_layout.csv"
...
def main() -> None:
    ...
    for strategy in available_ordering_strategies():
        ...
        # circuit qubit i -> physical qubit i.
        # Since the QUBO was relabeled by ordering, this makes ordering matter.
    ...
    out["delta_depth_vs_natural"] = (
        out["transpiled_depth"] - natural["transpiled_depth"]
    )
    out["delta_swap_vs_natural"] = (
        out["swap_count"] - natural["swap_count"]
    )

    out["pct_depth_change_vs_natural"] = (
        100.0 * out["delta_depth_vs_natural"] / natural["transpiled_depth"]
    )

    if natural["swap_count"] != 0:
        out["pct_swap_change_vs_natural"] = (
            100.0 * out["delta_swap_vs_natural"] / natural["swap_count"]
        )
    else:
        out["pct_swap_change_vs_natural"] = None

    out.to_csv(OUTPUT_PATH, index=False)

    print("=" * 80)
    print("RQ2 fixed-layout single-instance ordering comparison saved")
    print("=" * 80)
    ...
    print(
        out[
            [
                ...
                "pct_depth_change_vs_natural",
                "pct_swap_change_vs_natural",
            ]
